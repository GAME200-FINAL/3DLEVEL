...
public class TriggerReceiver : MonoBehaviour
{
    ...
    private bool objectInitVisibility;
    ...
    void Start()
    {
        if (GetComponent<GenericTrigger>().targetObject)
        {
            ...
            objectInitVisibility = target.activeSelf;
        }
    ...
    public void ToggleTrigger()
    {
        ...
        switch (selectedEvent)
        {
            ...
            case TriggerEvents.MoveBox:
                ToggleMoveBox();
                break;
            case TriggerEvents.ToggleObjectOn:
                ToggleObjectOn();
                break;
        }
    ...
    {
        if (Mathf.Abs(target.transform.position.x - nextWp.position.x) < .5f
        && Mathf.Abs(target.transform.position.y - nextWp.position.y) < .5f
        && Mathf.Abs(target.transform.position.z - nextWp.position.z) < .5f)
        {
    ...
    {
        if (Input.GetKeyDown(KeyCode.Q) && canBePushed)
        {
        ...
        if (Input.GetKeyDown(KeyCode.E) && canBePulled)
        {
            if (moveSpeed < 0)
                moveSpeed *= -1;
            moveBoxDirection = -1;
    ...
    private void InitialBoxMovement()
    {
        ...
        if (moveAlongX_Axis)
        {
            ...
            {
                moveSpeed *= -1;
                boxDistination = target.transform.position + new Vector3(-moveBoxDirection * moveDistance, 0, 0);
            }
            else
            {
                boxDistination = target.transform.position + new Vector3(moveBoxDirection * moveDistance, 0, 0);
        ...
        else
        {
            ...
            {
                moveSpeed *= -1;
                boxDistination = target.transform.position + new Vector3(0, 0, -moveBoxDirection * moveDistance);
            }
            else
            {
                boxDistination = target.transform.position + new Vector3(0, 0, moveBoxDirection * moveDistance);
    ...
    private void MoveBox()
    {
        if (moveAlongX_Axis)
        {
            ...
            {
                target.transform.position += new Vector3(moveBoxDirection * moveSpeed * Time.deltaTime, 0, 0);
                getPlayerTransform().position += new Vector3(moveBoxDirection * moveSpeed * Time.deltaTime, 0, 0);
            }
            else
                startMoveBox = false;
        }
        ...
        else
        {
            ...
            {
                target.transform.position += new Vector3(0, 0, moveBoxDirection * moveSpeed * Time.deltaTime);
                getPlayerTransform().position += new Vector3(0, 0, moveBoxDirection * moveSpeed * Time.deltaTime);
            }
            else
                startMoveBox = false;
        }
...
public class TriggerReceiverInspector : Editor
{
